...
def prepare_ncf_data(data, item_features, user_to_index, item_to_index):
    """تحضير بيانات NCF"""
    user_indices = data['user_id'].map(user_to_index).values
    item_indices = data['item_id'].map(item_to_index).values
    ratings = data['rating'].values
    user_context = data[['age', 'gender', 'occupation']].values
    ...
    item_context = item_data[genre_columns].values

    context_features = np.concatenate([user_context, item_context], axis=1)
